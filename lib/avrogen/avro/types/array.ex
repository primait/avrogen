...
defimpl CodeGenerator, for: Array do
  ...
  # For arrays of records, inline the function capture directly:
  #   def drop_pii_<fn>(value) when is_list(value), do: Enum.map(value, &RecordMod.drop_pii/1)
  #
  # For arrays of other PII-containing types (arrays, maps, ...), generate a named helper
  # and let CodeGenerator.drop_pii recurse into it:
  #   def drop_pii_<fn>(value) when is_list(value), do: Enum.map(value, &__MODULE__.drop_pii_<fn>_items/1)
  #   <helper clauses for drop_pii_<fn>_items generated by CodeGenerator.drop_pii>
  def drop_pii(%Array{items_schema: items_schema}, function_name, global) do
    ...
    case resolved do
      ...
      _ ->
        inner_fn = :"#{function_name}_items"
        inner = CodeGenerator.drop_pii(items_schema, inner_fn, global)

        quote do
          def unquote(function_name)(value) when is_list(value),
            do: Enum.map(value, &(__MODULE__.unquote(inner_fn) / 1))
